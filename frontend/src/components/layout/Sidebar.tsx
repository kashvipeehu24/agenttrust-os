...
export const Sidebar: React.FC = () => {
  ...
  const underwritingNavItems = [
    { path: '/underwriting', label: 'Underwriting Hub', icon: LayoutDashboard },
    { path: '/underwriting/loan-application', label: 'Apply for Loan', icon: PlusCircle },
  ];

  const repaymentNavItems = [
    { path: '/repayment/dashboard', label: 'Repayment Center', icon: Activity },
    { path: '/repayment/wallet', label: 'Credit Wallet', icon: Wallet },
    { path: '/repayment/cashflow', label: 'Cash Flow', icon: Activity },
    { path: '/repayment/forecast', label: 'Forecasts', icon: History },
    { path: '/repayment/transactions', label: 'Transactions', icon: FileCheck2 },
    { path: '/repayment/revenue', label: 'Revenue Dashboard', icon: Wallet },
    { path: '/repayment/suggestions', label: 'AI Insights', icon: Cpu },
  ];

  const governanceNavItems = [
    { path: '/governance', label: 'Governance Panel', icon: Shield },
    { path: '/governance/security', label: 'Security Center', icon: ShieldCheck },
    { path: '/governance/mission-control', label: 'Mission Control', icon: Cpu },
    { path: '/governance/live-monitoring', label: 'Live Monitoring', icon: Activity },
    { path: '/governance/fraud-alerts', label: 'Fraud Alerts', icon: AlertTriangle },
  ];
      ...
      <div>
        ...
        <div className="px-3 py-4 space-y-4">
          ...
          <div className="space-y-1 border-t border-slate-900 pt-2 mt-2">
            ...
            {underwritingNavItems.map((item) => (
              <SidebarItem key={item.path} item={item} />
            ))}
          </div>

          {/* Repayment Monitoring Module */}
          <div className="space-y-1 border-t border-slate-900 pt-2 mt-2">
            <div className="px-3 pb-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
              Repayment
            </div>
            {repaymentNavItems.map((item) => (
              <SidebarItem key={item.path} item={item} />
            ))}
          </div>
